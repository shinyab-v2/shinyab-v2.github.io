const STORAGE_WORDS = "studentDailyEnglish.words.v1";
const STORAGE_PROGRESS = "studentDailyEnglish.progress.v1";
...
function exportWords() {
  ...
  link.download = `student-daily-english-vocabulary-${new Date().toISOString().slice(0, 10)}.csv`;
  link.click();
...
function buildVoicePrompt(lesson) {
  ...
  return `You are a warm British English speaking coach for a Korean Grade 5–6 learner at A1–A2 level. The learner already completed this Student Daily English lesson on mobile. Now conduct a focused 10–12 minute review in Voice.

Follow these rules strictly:
1. Use clear British English at a slow, natural speed. Use one short Korean explanation only when the learner is confused.
2. Ask exactly one short question at a time. Let the learner think and finish without interruption. If unsure, ask, “Have you finished?”
3. Accept a short A1 answer first. Then help the learner add one detail or reason for an A2 answer.
4. After each answer, give brief feedback in this order: meaning → grammar → a natural expression → one British pronunciation point.
5. Ask the learner to repeat the corrected sentence. Allow at most two tries, praise the useful part, and move on.
6. Never shame, rush, or compare the learner. Do not give the answer before the learner tries.
7. Review through speaking: warm-up, grammar, three practical expressions, reading comprehension, and pronunciation.
8. If pronunciation cannot be judged reliably, say so instead of inventing an error.
9. Finish with the role-play for two or three exchanges. At the end, give two important corrections and two items to practise again.
10. Begin immediately with one easy warm-up question connected to the reading. Do not read these rules aloud.
...
fetch("data/week.json")
  ...
  .then((data) => {
    if (!Array.isArray(data) || !data.length) throw new Error("No English lessons available");
    const today = localToday();
    allLessons = [...data].sort((a, b) => b.date.localeCompare(a.date));
